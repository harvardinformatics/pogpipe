...
// no importance to this module-- all that it important abotu it is represented
// by a segment's length and score.
//
...
//
// new_segment_table--
//	Allocate am empty segment table.
//
//----------
//
// Arguments:
...
// this is the same format as is written by write_segments (NOT the same format
// as is written by dump_segments).  There are three columns for the target--
// name, start, end.  These are followed by three columns for the query (with
// the same meaning), the query strand, and an optional score.
// 
...
//	pos1, pos2, length, s, id:	The segment to add.
//
// Returns:
...
//
// write_segments--
//	Write a segment table to a file.
//
//----------
//
// Arguments:
...
void dump_segments
   (FILE*		f,
	...
	segment*	seg;
	unspos		prevPos2;
	sgnpos		diag, prevDiag;

	sym1 = _sym1;
	sym2 = _sym2;

	prevDiag = 0;  prevPos2 = 0;
	for (ix=0,seg=st->seg ; ix<st->len ; ix++,seg++)
		{
		if (_sym1 == NULL) sym1 = "+";
